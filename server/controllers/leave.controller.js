...
import { getLeaveBalanceSummary, recalculateEmployeeYearLeave, setAvailablePaidLeave } from "../services/leave/leaveBalance.service.js";
 ...
 * Body: { year?: number, availablePaidLeave: number }
 *
 * Admin correction of the available paid leave (whole days), used when an
 * employee already took leave that the system does not know about.
 */
export async function updateLeaveBalance(req, res) {
    try {
        const { employeeId } = req.params;
        const year = Number(req.body.year || new Date().getFullYear());
        const availablePaidLeave = Number(req.body.availablePaidLeave);
        ...
        if (
            req.body.availablePaidLeave === undefined ||
            req.body.availablePaidLeave === "" ||
            !Number.isInteger(availablePaidLeave) ||
            availablePaidLeave < 0 ||
            availablePaidLeave > 365
        ) {
            return res.status(400).json({
                success: false,
                message: "Available leave must be a whole number of days between 0 and 365",
            });
        }
        ...
        const balance = await setAvailablePaidLeave(employeeId, year, availablePaidLeave);
        return res.status(200).json({
            success: true,
