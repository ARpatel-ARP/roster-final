...
function TeamsPage() {
  ...
  // team id -> sorted list of member first names
  const memberFirstNamesByTeam = useMemo(() => {
    const employees = employeesResponse?.data || [];
    const map = {};

    for (const employee of employees) {
      const teamId = String(employee.team?._id || employee.team || "");
      const firstName = (employee.name || "").trim().split(/\s+/)[0];
      if (!teamId || !firstName) continue;

      (map[teamId] ||= []).push(firstName);
    }

    for (const teamId of Object.keys(map)) {
      map[teamId].sort((a, b) => a.localeCompare(b));
    }
  ...
  const getEmployeeCount = (teamId) =>
    memberFirstNamesByTeam[String(teamId)]?.length || 0;

  const getMemberNames = (teamId) =>
    (memberFirstNamesByTeam[String(teamId)] || []).join(", ");
  ...
  <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">
    {getEmployeeCount(team._id)}
                            ...
                            <p
                              title={getMemberNames(team._id)}
                              className="mt-1 max-w-lg truncate text-sm text-slate-500"
                            >
                              {getMemberNames(team._id)
                                ? `(${getMemberNames(team._id)})`
                                : "(No members)"}
                            </p>
                          </Link>
