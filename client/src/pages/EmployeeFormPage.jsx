...
function EmployeeFormPage() {
  ...
  // Leave balance correction (edit mode only)
  const currentYear = new Date().getFullYear();
  const [availableLeave, setAvailableLeave] = useState("");
  const leaveInitialised = useRef(false);
  ...
  const { data: leaveBalanceResponse } = useGetLeaveBalanceQuery(
    { employeeId: id, year: currentYear },
    { skip: !isEditMode }
  );
  const leaveBalance = leaveBalanceResponse?.data;

  const [updateLeaveBalance, { isLoading: isUpdatingLeave }] =
    useUpdateLeaveBalanceMutation();
  ...
  // Pre-fill the leave field once with the current available balance.
  useEffect(() => {
    if (!leaveBalance || leaveInitialised.current) return;
    leaveInitialised.current = true;
    setAvailableLeave(String(leaveBalance.availablePaidLeave ?? 0));
  }, [leaveBalance]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate the leave override before saving anything.
    const leaveChanged =
      isEditMode &&
      leaveBalance &&
      availableLeave !== "" &&
      Number(availableLeave) !== leaveBalance.availablePaidLeave;

    if (isEditMode && availableLeave !== "") {
      const value = Number(availableLeave);
      if (!Number.isInteger(value) || value < 0 || value > 365) {
        setError("Available leave must be a whole number of days (0 or more).");
        return;
      }
    }
      ...
      if (isEditMode) {
        ...
        if (leaveChanged) {
          await updateLeaveBalance({
            employeeId: id,
            year: currentYear,
            availablePaidLeave: Number(availableLeave),
          }).unwrap();
        }
  ...
  const isSubmitting = isCreating || isUpdating || isUpdatingLeave;
              ...
              <div className="grid grid-cols-1 gap-5 p-5 sm:grid-cols-2 sm:p-6">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setAvailableLeave(e.target.value)}
                    disabled={!leaveBalance}
                    placeholder={leaveBalance ? "e.g. 8" : "Loading..."}
                    className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-slate-400 disabled:cursor-not-allowed disabled:bg-slate-100"
                  />

                  <p className="mt-1.5 text-xs text-slate-400">
                    Set this to the number of leave days the employee actually
                    has left today (whole days). Use it when leave was taken
                    before this system was in use. Monthly accrual and any
                    leave applied later continue on top of this value.
                  </p>
                </div>
                ...
                {leaveBalance && (
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                    ...
                    <ul className="space-y-1">
                      <li>Accrued so far: {leaveBalance.accrued}</li>
                      <li>Carried forward: {leaveBalance.carriedForward}</li>
                      <li>Leave used in system: {leaveBalance.paidLeaveUsed}</li>
                      <li>
                        Manual adjustment:{" "}
                        {leaveBalance.manualAdjustment > 0 ? "+" : ""}
                        {leaveBalance.manualAdjustment}
                      </li>
                      <li className="font-medium text-slate-700">
                        Currently available: {leaveBalance.availablePaidLeave}
                      </li>
                    </ul>
                  </div>
                )}
